# scheduler-cli — Spec Deltas (scheduler-cli-7)

> Ticket: 7 · Capability: `scheduler-cli` (NEW) · deltaMode: ADDED
> S2 functional spec: full happy + error AC scenarios, business rules, integration points.
...
- **BR-7-001**: A recoverable per-repo collector error (`InvalidRepoError`, `NetworkError`) is logged and skipped; it never aborts the run.
- **BR-7-002**: `AuthError` is always fatal (exit 1) — never retried, never skipped — because all repos share one token.
- **BR-7-003**: Every collected item is recorded via `mark_seen`; `first_seen_at` is write-once and never overwritten on re-mark (inherited from state-store-3).
- **BR-7-004**: Handled errors surface as `Error: <message>` on stderr with exit 1 and no traceback; success exits 0.
- **BR-7-005**: Exactly one outcome log line is emitted per repo; secrets are never logged.
- **BR-7-006**: `run_pipeline` constructs adapters from `Config` only; the token goes only to the collector and the api-key only to the summarizer — neither is stored on the pipeline object.
- **BR-7-007**: One run delivers exactly one digest (a single `deliver(content)` call), never one-per-repo.
- **BR-7-008**: A terminal `RateLimitError` stops further collection but still delivers items already collected (exit 0).
- **BR-7-009**: The pipeline calls `summarize_items()` exactly once per run when an LLM provider is configured.
- **BR-7-010**: When `llm_provider is None`, the summarizer is neither constructed nor called; each item's `summary` is the fixed placeholder `"(no summary — LLM disabled)"` so the rendered digest visibly marks that summaries were intentionally skipped (not a failure). The placeholder is non-empty, so the renderer emits it on each item line (renderer omits only empty/whitespace summaries, AC-5-017).
- **BR-7-011**: V1 records seen state but does not suppress seen items from the digest (suppression is V2).
- **BR-7-012**: No `github_token` or `llm_api_key` value ever appears in stderr, logs, or the delivered digest.

## Integration Points

- **INT-7-001**: Consumes the delivery-6 CLI error contract (`Error: <msg>` / exit codes / `BrokenPipeError` handling) in `cli.py`.
- **INT-7-002**: Consumes `GitHubCollector.fetch_items(repo: str, lookback_days: int) -> list[RawItem]` (github-collector-2).
- **INT-7-003**: Consumes `JsonFileStateStore.mark_seen(...)` + atomic `save` (state-store-3).
- **INT-7-004**: Consumes `LiteLLMSummarizer.summarize_items(list[RawItem]) -> list[SummarizedItem]` (summarizer-llm-4).
- **INT-7-005**: Consumes `render(items: list[SummarizedItem], *, lookback_days: int) -> str` (digest-renderer-5).
- **INT-7-006**: Consumes `FileDelivery`/`StdoutDelivery.deliver(content: str) -> None` (delivery-6).

## _Structured Extract

```yaml
ticket_id: 7
change_name: scheduler-cli-7
...
acceptance_criteria:
  ...
  missing: 0
  unclear: 0
  ids: [AC-7-001, AC-7-002, AC-7-003, AC-7-004, AC-7-005, AC-7-006, AC-7-007, AC-7-008, AC-7-009, AC-7-010, AC-7-011, AC-7-012, AC-7-013, AC-7-014, AC-7-015, AC-7-016, AC-7-017, AC-7-018, AC-7-019, AC-7-020, AC-7-021, AC-7-022]
business_rules:
  total: 12
  ids: [BR-7-001, BR-7-002, BR-7-003, BR-7-004, BR-7-005, BR-7-006, BR-7-007, BR-7-008, BR-7-009, BR-7-010, BR-7-011, BR-7-012]
integration_points:
  total: 6
  ids: [INT-7-001, INT-7-002, INT-7-003, INT-7-004, INT-7-005, INT-7-006]
decisions: [D-1]
edge_cases: 20
risk_flags: [RF-1, RF-2, RF-3, RF-4, RF-5, RF-6]
stride_gate: PASS
figma: N/A
```
